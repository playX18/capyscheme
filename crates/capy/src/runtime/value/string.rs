...
unsafe impl ClassTagged for Stringbuf {
    const CLASS_IDS: &'static [u32] = &[
        crate::heap::object::builtin_class_ids::STRINGBUF_WIDE,
        crate::heap::object::builtin_class_ids::STRINGBUF_NARROW,
    ];
...
extern "C" fn process_weak_stringbuf(_: GcObject, _: &mut crate::heap::WeakProcessor) {}
...
impl Stringbuf {
    ...
    #[allow(dead_code)]
    pub(crate) fn wide<'gc>(&self, mc: impl Into<Mutation<'gc>>) -> Gc<'gc, Self> {
        let mc = mc.into();
        if self.is_wide() {
    ...
    fn narrow<'gc>(&self, mc: impl Into<Mutation<'gc>>) -> Gc<'gc, Self> {
        let mc = mc.into();
        if self.is_narrow() {
            // SAFETY: The pointer references a valid GC-managed object of the expected type
            return unsafe { Gc::from_ptr(self) };
...
fn null_stringbuf<'gc>(mc: impl Into<Mutation<'gc>>) -> Gc<'gc, Stringbuf> {
    let mc = mc.into();
    *NULL_STRINGBUF
...
impl<'gc> Str<'gc> {
    ...
    pub fn from_str(mc: impl Into<Mutation<'gc>>, s: impl AsRef<str>) -> Gc<'gc, Self> {
        let mc = mc.into();
        Self::new(mc, s, false)
    }
    ...
    pub fn from_char(mc: impl Into<Mutation<'gc>>, c: char, count: usize) -> Gc<'gc, Self> {
        let mc = mc.into();
        if c as u32 <= 0xff {
            let buf = if count == 0 {
                null_stringbuf(mc)
    ...
    pub fn try_narrow(this: Gc<'gc, Self>, mc: impl Into<Mutation<'gc>>) -> bool {
        let mc = mc.into();
        if this.is_narrow() {
            return true;
        }
    ...
    pub fn try_as_str(this: Gc<'gc, Self>, mc: impl Into<Mutation<'gc>>) -> Option<&'gc str> {
        let mc = mc.into();
    ...
    pub fn ensure_mutable(this: Gc<'gc, Self>, mc: impl Into<Mutation<'gc>>) -> bool {
        let mc = mc.into();
        if !this.is_mutable() {
            return false;
        }
    ...
    pub fn set(this: Gc<'gc, Self>, mc: impl Into<Mutation<'gc>>, index: usize, value: char) {
        let mc = mc.into();
        ...
        if this.is_narrow() {
            this.chars_mut()
                .expect("string buffer encoding matches branch")[index] = value as u8;
        } else {
            this.wide_chars_mut()
                .expect("string buffer encoding matches branch")[index] = value;
        }
    }
...
unsafe impl<'gc> ClassTagged for Str<'gc> {
    const CLASS_IDS: &'static [u32] = &[crate::heap::object::builtin_class_ids::STRING];
...
impl<'gc> Ord for Str<'gc> {
    fn cmp(&self, other: &Self) -> std::cmp::Ordering {
        compare_strings(false, self, other, None, None, None, None)
            .expect("string compare always yields ordering")
    }
}
...
impl<'gc> Hash for Str<'gc> {
    fn hash<H: std::hash::Hasher>(&self, state: &mut H) {
        if self.is_narrow() {
            ...
            for c in self
                .wide_chars()
                .expect("string buffer encoding matches branch")
            {
