...
use crate::heap::{Gc, Trace, collection::Visitor};
...
fn complex_header_word() -> u64 {
    class_header_word(
        ClassId::new(builtin_class_ids::COMPLEX).expect("builtin class id is nonzero"),
    )
}
...
fn rational_header_word() -> u64 {
    class_header_word(
        ClassId::new(builtin_class_ids::RATIONAL).expect("builtin class id is nonzero"),
    )
}
...
impl<'gc> Number<'gc> {
    ...
    pub fn reduce(ctx: Context<'gc>, numerator: Self, denominator: Self) -> Self {
        ...
        if let Number::Fixnum(nume) = numerator {
            ...
            return Self::reduce_fix_big(
                ctx,
                nume,
                denominator.as_bigint().expect("value is bigint"),
            );
        }

        if let Number::Fixnum(deno) = denominator {
            return Self::reduce_big_fix(
                ctx,
                numerator.as_bigint().expect("value is bigint"),
                deno,
            );
        }
...
impl<'gc> Rational<'gc> {
    ...
    pub fn to_f64(&self, ctx: Context<'gc>) -> f64 {
        ...
        if nume.is_infinite() || deno.is_infinite() {
            if nume.is_infinite() && deno.is_infinite() {
                let nume_bitsize =
                    BigInt::bitsize(self.numerator.as_bigint().expect("value is bigint"));
                let deno_bitsize =
                    BigInt::bitsize(self.denominator.as_bigint().expect("value is bigint"));
                ...
                nume = BigInt::shift_right(
                    self.numerator.as_bigint().expect("value is bigint"),
                    ctx,
                    shift as _,
                )
                .as_f64();
                deno = BigInt::shift_right(
                    self.denominator.as_bigint().expect("value is bigint"),
                    ctx,
                    shift as _,
                )
                .as_f64();
