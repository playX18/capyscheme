...
impl<'gc> Number<'gc> {
    pub fn integer_to_string(self, ctx: impl Into<Mutation<'gc>>, radix: u8) -> String {
        let ctx = ctx.into();
        if !self.is_exact() {
    ...
    pub fn flonum_to_string(self, ctx: impl Into<Mutation<'gc>>, radix: u8) -> String {
        let ctx = ctx.into();
    ...
    pub fn compnum_to_string(self, ctx: impl Into<Mutation<'gc>>, radix: u8) -> String {
        let ctx = ctx.into();
        let c = match self {
            Self::Complex(c) => c,
            _ => unreachable!(),
    ...
    pub fn ratnum_to_string(self, ctx: impl Into<Mutation<'gc>>, radix: u8) -> String {
        let ctx = ctx.into();
        let r = match self {
            Self::Rational(r) => r,
            _ => unreachable!(),
    ...
    pub fn make_rectangular(ctx: impl Into<Mutation<'gc>>, real: Self, imag: Self) -> Self {
        let ctx = ctx.into();
    ...
    pub fn numerator(self, ctx: impl Into<Mutation<'gc>>) -> Self {
        let ctx = ctx.into();
    ...
    pub fn denominator(self, ctx: impl Into<Mutation<'gc>>) -> Self {
        let ctx = ctx.into();
        let inexact = !self.is_exact();
        let obj = self.to_exact(ctx);
        match obj {
    ...
    pub fn to_string_radix(self, ctx: impl Into<Mutation<'gc>>, base: u8) -> String {
        let ctx = ctx.into();
        match self {
