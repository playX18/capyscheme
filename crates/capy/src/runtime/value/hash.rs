...
impl<'gc> HashTable<'gc> {
    ...
    pub fn lock(&'gc self) -> crate::heap::sync::monitor::MonitorGuard<'gc, InnerHashTable<'gc>> {
        self.inner.lock()
    }
...
unsafe impl<'gc> ClassTagged for HashTable<'gc> {
    const CLASS_IDS: &'static [u32] = &[
        crate::heap::object::builtin_class_ids::HASHTABLE,
        crate::heap::object::builtin_class_ids::IMMUTABLE_HASHTABLE,
    ];
