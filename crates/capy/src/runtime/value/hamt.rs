...
fn hamt_node_header_word() -> u64 {
    class_header_word(
        ClassId::new(builtin_class_ids::HAMT_NODE).expect("builtin class id is nonzero"),
    )
}
...
impl<'gc> HamtTrie<'gc> {
    pub fn new(_mc: Context<'gc>, typ: HashTableType<'gc>, kind: HamtKind) -> Self {
        Self {
            root: Lock::new(None),
            count: 0,
            typ,
...
impl<'gc> PersistentMap<'gc> {
    pub fn new(mc: Context<'gc>, typ: HashTableType<'gc>) -> Gc<'gc, Self> {
