...
//! must be reloaded after any safepoint. Prefer OopStorage / [`crate::runtime::sni`]
...
impl<'a, 'gc> RootScope<'a, 'gc> {
    ...
    pub fn root_gc<T: Trace + 'gc>(&self, gc: crate::heap::Gc<'gc, T>) -> Rooted<'a, 'gc>
    where
        crate::heap::Gc<'gc, T>: Into<Value<'gc>>,
    {
        self.root(gc.into())
    }
}
