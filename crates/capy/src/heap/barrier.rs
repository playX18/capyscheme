...
///
/// ```
/// # use capy::heap::barrier::{field, Write};
/// struct Container<T> {
///     field: T,
/// }
///
...
macro_rules! __field {
    ($value:expr, $type:path, $field:ident) => {
        ...
        match $value {
            &$crate::heap::barrier::Write {
                __inner: $type { ref $field, .. },
                ..
            } => unsafe {
                $crate::heap::barrier::Write::__from_ref_and_ptr($field, $field as *const _)
            },
        }
    };
    ...
    ($value:expr, $type:path, $field:ident[$index: expr]) => {
        ...
        match $value {
            &$crate::heap::barrier::Write {
                ...
                $crate::heap::barrier::Write::__from_ref_and_ptr(
                    &$field[$index],
...
macro_rules! __unlock {
    ($value:expr, $type:path, $field:ident) => {
        $crate::heap::barrier::field!($value, $type, $field).unlock()
    };
}
