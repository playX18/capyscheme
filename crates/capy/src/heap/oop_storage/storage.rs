...
impl OopStorage {
    ...
    pub fn allocate_bulk(&self, ptrs: &mut [*mut Value<'static>]) -> usize {
        ...
        self.allocation_count
            .fetch_add(num_taken, Ordering::Relaxed);
    ...
    pub fn scan_weak_roots<A>(
        &self,
        mut is_alive: A,
        factory: &mut impl RootsWorkFactory<ObjectSlot>,
    ) where
        A: FnMut(Value<'static>) -> bool,
    {
        let mut tracer = RootSlotVisitor::new();
...
impl Drop for OopStorage {
    fn drop(&mut self) {
        ...
        while !block.is_null() {
            ...
            let next = if next == block { ptr::null_mut() } else { next };
            unsafe { (*block).set_deferred_updates_next(ptr::null_mut()) };
...
mod tests {
    use super::*;
    use crate::heap::oop_storage::block::BULK_ALLOCATE_LIMIT;
    use crate::heap::oop_storage::par_state::ParState;
    ...
    fn allocation_status_roundtrip() {
        ...
        let ptr = storage.allocate().unwrap();
        assert_eq!(storage.allocation_status(ptr), EntryStatus::Allocated);
        unsafe {
            *ptr = Value::empty();
            storage.release(ptr);
        }
        assert_eq!(storage.allocation_status(ptr), EntryStatus::Unallocated);
    }
}
