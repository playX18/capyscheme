use crate::heap::collection::{Visitor, VisitorKind};
use crate::heap::{Gc, mm::MemoryManager, object::GcObject, traits::Trace};
use crate::heap::{Mutation, ObjectSlot};
use mmtk::{
    scheduler::GCWorker,
...
impl<'gc, T> Weak<'gc, T> {
    ...
    pub fn upgrade(self, mc: impl Into<Mutation<'gc>>) -> Option<Gc<'gc, T>> {
        let mc = mc.into();
