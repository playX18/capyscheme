...
use crate::heap::collection::Visitor;
use crate::heap::{Mutation, NarrowGc, ObjectSlot};
use crate::heap::{
    ptr::Gc,
...
pub unsafe trait Trace {
    ...
    ///
    /// Prefer [`Self::trace_weak`] at call sites; the default is a no-op for types without weaks.
    unsafe fn process_weak_refs(&mut self, _weak_processor: &mut WeakProcessor) {}

    /// Weak-reference processing during `VMRefClosure` (alias of [`Self::process_weak_refs`]).
    unsafe fn trace_weak(&mut self, weak_processor: &mut WeakProcessor) {
        unsafe { self.process_weak_refs(weak_processor) }
    }
}

unsafe impl<'gc, T> Trace for Gc<'gc, T> {
    unsafe fn trace(&mut self, visitor: &mut Visitor) {
        visitor.visit_slot(crate::heap::ObjectSlot::from_address(Address::from_ref(
            self,
        )));
    }
...
pub trait FromIteratorGc<'gc, A>: Sized {
    fn from_iter_gc<T>(mc: impl Into<Mutation<'gc>>, iter: T) -> Gc<'gc, Self>
    where
...
pub trait IterGc<'gc> {
    ...
    fn collect_gc<T>(self, mc: impl Into<Mutation<'gc>>) -> Gc<'gc, T>
    where
...
impl<'gc, T: Iterator<Item = A>, A: Trace> IterGc<'gc> for T {
    ...
    fn collect_gc<I>(self, mc: impl Into<Mutation<'gc>>) -> Gc<'gc, I>
    where
