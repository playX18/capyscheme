...
impl<'gc, T> Gc<'gc, T> {
    ...
    #[inline]
    pub fn new(mc: impl Into<Mutation<'gc>>, value: T) -> Gc<'gc, T>
    where
        T: Trace,
    {
        mc.into()
            .allocate(value, mmtk::AllocationSemantics::Default)
    }

    #[inline]
    pub fn new_with_header_word(
        mc: impl Into<Mutation<'gc>>,
        value: T,
        header_word: u64,
    ) -> Gc<'gc, T> {
        mc.into()
            .allocate_with_header_word(value, header_word, mmtk::AllocationSemantics::Default)
    }
    ...
    pub fn write(mc: impl Into<Mutation<'gc>>, this: Self) -> &'gc Write<T> {
        let mc = mc.into();
        unsafe {
    ...
    pub fn compress(mc: impl Into<Mutation<'gc>>, this: Self) -> NarrowGc<'gc, T> {
        let mc = mc.into();
...
impl<'gc, T: Unlock + 'gc> Gc<'gc, T> {
    pub fn unlock(self, mc: impl Into<Mutation<'gc>>) -> &'gc T::Unlocked {
        Gc::write(mc, self);
...
impl<'gc, T> NarrowGc<'gc, T> {
    ...
    #[inline(always)]
    pub fn decompress(self, mc: impl Into<Mutation<'gc>>) -> Gc<'gc, T> {
        let mc = mc.into();
        unsafe {
            let thread = mc.thread();
