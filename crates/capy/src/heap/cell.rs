...
impl<'gc, T: Copy + 'gc> Gc<'gc, Lock<T>> {
    ...
    #[inline]
    pub fn set(self, mc: impl Into<Mutation<'gc>>, t: T) {
...
impl<'gc, T: Sized + 'gc> Gc<'gc, RefLock<T>> {
    ...
    pub fn borrow(self) -> Ref<'gc, T> {
        RefLock::borrow(self.as_gc_ref())
    }

    #[inline]
    pub fn try_borrow(self) -> Result<Ref<'gc, T>, std::cell::BorrowError> {
        RefLock::try_borrow(self.as_gc_ref())
    }

    #[track_caller]
    #[inline]
    pub fn borrow_mut(self, mc: impl Into<Mutation<'gc>>) -> RefMut<'gc, T> {
