...
impl<'gc> Proc<'gc> {
    pub fn with_body(&self, mc: Context<'gc>, body: TermRef<'gc>) -> ProcRef<'gc> {
        Gc::new(
            mc,
            Proc {
                name: self.name,
...
impl<'gc> Let<'gc> {
    pub fn with_body(&self, _mc: Context<'gc>, body: TermRef<'gc>) -> Let<'gc> {
        Let {
            style: self.style,
            lhs: self.lhs,
            rhs: self.rhs,
...
impl<'gc> Fix<'gc> {
    pub fn with_body(&self, _mc: Context<'gc>, body: TermRef<'gc>) -> Fix<'gc> {
        Fix {
            lhs: self.lhs,
            rhs: self.rhs,
            body,
