...
use crate::{
    ...
    heap::object::{OBJECT_HEADER_OFFSET, builtin_class_ids},
    runtime::{
...
impl<'gc> ModuleBuilder<'gc> {
    ...
    fn build_wrong_arity_trampoline(
        ...
        builder
            .ins()
            .store(mem_tm, overflow, state, offset_of!(State, runstack) as i32);
    ...
    fn build_generic_apply_trampoline_inner(
        ...
        for offset in [
            ...
            builder
                .ins()
                .store(mem_tm, undefined, state, cdata + offset as i32);
        }
