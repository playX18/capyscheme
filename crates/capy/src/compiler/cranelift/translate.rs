...
use crate::{
    compiler::{
        ...
        cps::graph::Atom,
        cranelift::{
            MAX_RAISE_ARITY, RegisterCallArgs, RestSource, SsaBuilder, VarDef,
            primitive::PrimValue, scheme_call_values,
        },
    },
...
impl<'gc, 'a, 'f> SsaBuilder<'gc, 'a, 'f> {
    ...
    pub fn prepare_call_args(&mut self, args: &[ir::Value]) -> RegisterCallArgs {
        ...
        let overflow = if overflow_count == 0 {
            ...
            let new_runstack = self
                .builder
                .ins()
                .iadd_imm_s(runstack, (overflow_count * size_of::<Value>()) as i64);
            self.builder.ins().store(
                ir::MemFlagsData::trusted().with_can_move(),
                new_runstack,
                state,
    ...
    /// defs must not outlive this join. GC relocates the `gc_save` slots in
    /// place; live values after the yield come only from the reloaded slots.
    pub fn check_yield(
        &mut self,
        rator: ir::Value,
        argc: ir::Value,
        ...
        let take_yieldpoint = self
            .builder
            .ins()
            .icmp_imm_s(IntCC::NotEqual, yieldpoint, 0);
        let join_args = [
        ...
        self.builder.switch_to_block(on_yieldpoint);
        {
            ...
            self.builder
                .ins()
                .call(self.thunks.yieldpoint_block, &[ctx]);
