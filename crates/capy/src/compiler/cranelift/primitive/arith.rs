...
pub fn lower_sqrt<'gc_, 'a, 'f>(
    ...
    let result = ssa.inline_float_unary_op(
        arg,
...
pub fn lower_numeric_lte<'gc_, 'a, 'f>(
    ...
    for arg in &args[1..] {
        ...
        if ssa.builder.func.dfg.value_type(lte) != ssa.builder.func.dfg.value_type(acc) {
            unreachable!(
                "type mismatch in <=: {:?} vs {:?}, args: {:?}",
                ssa.builder.func.dfg.value_type(lte),
                ssa.builder.func.dfg.value_type(acc),
                args
            )
        }
...
pub(super) fn ensure_bytevector<'gc, 'a, 'f>(
    ssa: &mut SsaBuilder<'gc, 'a, 'f>,
    val: ir::Value,
    on_bytevector: impl FnOnce(&mut SsaBuilder<'gc, 'a, 'f>, ir::Value, ir::Block),
    slowpath: impl FnOnce(&mut SsaBuilder<'gc, 'a, 'f>, ir::Value),
) {
    let bb_bv = ssa.builder.create_block();
    let bb_slow = ssa.builder.create_block();

    ssa.builder.func.layout.set_cold(bb_slow);

    ssa.branch_if_any_heap_class_id(
        val,
        &[
    ...
    ssa.builder.switch_to_block(bb_bv);
    {
        let length = ssa.builder.ins().load(
            types::I64,
            ir::MemFlagsData::trusted().with_can_move(),
        ...
        on_bytevector(ssa, length, bb_slow);
    }
    ssa.builder.switch_to_block(bb_slow);
    {
        slowpath(ssa, val);
