    ...
    let collect_impl = {
        ...
        if let Some(_gc_lifetime) = gc_lifetime {
            impl_struct.gen_impl(quote! {
                gen unsafe impl  crate::heap::Trace for @Self #where_clause {
                    #[inline]
                    unsafe fn trace(&mut self, vis: &mut   crate::heap::collection::Visitor) {
                        match *self { #trace_body }
                    }

                    #[inline]
                    unsafe fn process_weak_refs(&mut self, weak_processor: &mut   crate::heap::weak::WeakProcessor) {
            ...
            impl_struct.gen_impl(quote! {
                gen unsafe impl<'gc>   crate::heap::Trace for @Self #where_clause {
