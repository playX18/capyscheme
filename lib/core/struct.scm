      ...
      (syntax-case x ()
        ((?_ ?struct-name ?field-names)
          (let ((struct-name (syntax->datum #'?struct-name))
            ...
            (let ((make-name (string->symbol (format #f "make-~a" struct-name)))
                  (pred-name (string->symbol (format #f "~a?" struct-name)))
                  (desc-name (string->symbol (format #f "type:~a" struct-name)))
              ...
              (let ((field-refs (iota field-count 1)))
                (with-syntax
                  ((?make-name
                    ...
                    ((?setter-name ...)
                      (map (lambda (field-name)
                            (datum->syntax #'?_ (string->symbol (format #f "~a-~a-set!" struct-name field-name))))
                        field-names))
                    ((?getter-rules ...)
