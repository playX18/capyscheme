              ...
              (cond ((= byte1 0)
                ...
                (else
                  (let ((byte3 (bytevector-ref buffer (+ i 2 offset1)))
                    ...
                    (if (<= #xdc byte3 #xdf)
                      (let* ((bits (fxior
                                    (ash (logand byte1 3) 18)
                                    (ash byte2 10)
                                    (ash (logand byte3 3) 8)
                                    byte4))
                             (sv (+ bits #x10000))
                             (b1 (logior #b11110000
                                  (ash sv -18)))
          ...
          (cond ((<= byte1 #x7f)
            ...
            ((<= byte1 #xdf)
              (let ((byte2 (bytevector-ref bv (+ i 1))))
                (write16 (logand #b00000111 (ash byte1 -2))
                  (logior (logand #b11000000 (ash byte1 6))
                    (logand #b00111111 byte2)))
                (loop (+ i 2) limit)))
            ((= (+ i 2) limit)
              (return i))
            ((<= byte1 #xef)
              (let ((byte2 (bytevector-ref bv (+ i 1)))
                ...
                (write16 (logior
                          (logand #b11110000 (ash byte1 4))
                          (logand #b00001111 (ash byte2 -2)))
                  (logior (logand #b11000000 (ash byte2 6))
                    (logand #b00111111 byte3)))
                (loop (+ i 3) limit)))
            ((= (+ i 3) limit)
              (return i))
            (else
              (let* ((byte2 (bytevector-ref bv (+ i 1)))
                     ...
                     (sv (fxior
                          (ash (logand byte1 #b00000111) 18)
                          (ash (logand byte2 #b00111111) 12)
                          (ash (logand byte3 #b00111111) 6)
                          (logand byte4 #b00111111)))
                     (bits (- sv #x10000)))
                (write16 (logior #xd800 (ash bits -10))
                  (logior #xdc00 (logand bits #x03ff)))
                (loop (+ i 4) limit)))))))
