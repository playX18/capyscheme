...
(define (print x p slashify)
    ...
    (define (print-in-string-without-hexifying? c)
        (unless (char? c)
            (error 'print-in-string-without-hexifying? "not a character" c))
    ...
    (define (print-as-char-without-hexifying? c)
        (unless (char? c)
            (error 'print-as-char-without-hexifying? "not a character" c))
        (let ((sv (char->integer c)))
            (or (<= 32 sv 126)
                (and (<= 128 sv)
                (not (memq (char-general-category c)
  ...
  (define (print-slashed-symbol-string s p r7rs?)
    ...
    (define (printcharacter c p)
        (unless (char? c)
            (error 'printcharacter "not a character" c))
        (write-char #\# p)
        (write-char #\\ p)
