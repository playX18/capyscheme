...
(define (%read-error? x) (read-error? x))

(define (assume test message . irritants)
    (if (not test)
        (error 'assume message irritants)
        #t))
                               ...
                               (if (char=? (generator) #\u)
                                   (let ((code2 (read-unicode-escape generator)))
                                     (if (<= #xdc00 code2 #xdfff)
                                         (let ((integer
                                                (+ #x10000 (bitwise-ior
                                                            (bitwise-arithmetic-shift (- code1 #xd800) 10)
                                                            (- code2 #xdc00)))))
