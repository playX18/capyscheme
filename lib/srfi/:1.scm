(define-library (srfi :1)
  ...
  (export xcons
    ...
    pair-fold
    reduce
    (rename (fold-right/1 fold-right))
    unfold-right
    pair-fold-right
    reduce-right
    append-map
  ...
  (begin
    (define for-each-1/1
      (lambda (proc lst)
        (cond ((null? lst) (unspecified)) (else (proc (car lst)) (for-each-1/1 proc (cdr lst))))))

    (define for-each-n/1
      (lambda (proc lst)
        (cond ((null? lst) (unspecified)) (else (apply proc (car lst)) (for-each-n/1 proc (cdr lst))))))

    (define for-each/1
      (lambda (proc lst1 . lst2)
        (if (null? lst2) (for-each-1/1 proc lst1) (for-each-n/1 proc (apply list-transpose* lst1 lst2)))))

    (define map-1/1
      (lambda (proc lst)
        (cond ((null? lst) '()) (else (cons (proc (car lst)) (map-1/1 proc (cdr lst)))))))

    (define map-n/1
      (lambda (proc lst)
        (cond ((null? lst) '()) (else (cons (apply proc (car lst)) (map-n/1 proc (cdr lst)))))))

    (define map/1
      (lambda (proc lst1 . lst2)
        (if (null? lst2) (map-1/1 proc lst1) (map-n/1 proc (apply list-transpose* lst1 lst2)))))
    ...
    (define unzip2
      (lambda (lst)
        (values (map-1/1 first lst) (map-1/1 second lst))))

    (define unzip3
      (lambda (lst)
        (values (map-1/1 first lst) (map-1/1 second lst) (map-1/1 third lst))))

    (define unzip4
      (lambda (lst)
        (values (map-1/1 first lst) (map-1/1 second lst) (map-1/1 third lst) (map-1/1 fourth lst))))
    ...
    (define pair-fold
      (lambda (proc seed lst1 . lst2)
        ...
        (if (null? lst2)
          ...
          (pair-fold-n proc seed (apply list-transpose* (list-of-subset lst1) (map-1/1 list-of-subset lst2))))))
      ...
      (lambda (proc seed lst1 . lst2)
        ...
        (if (null? lst2)
          ...
          (pair-fold-right-n
            proc
            seed
            (apply list-transpose* (list-of-subset lst1) (map-1/1 list-of-subset lst2))))))
    ...
    (define append-map
      (lambda (proc lst1 . lst2)
        (if (null? lst2)
          (apply append (map-1/1 proc lst1))
          (apply append (map-n/1 proc (apply list-transpose* lst1 lst2))))))
    ...
    (define pair-for-each
      (lambda (proc lst1 . lst2)
        (if (null? lst2)
          (for-each-1/1 proc (list-of-subset lst1))
          (for-each-n/1 proc (apply list-transpose* (list-of-subset lst1) (map list-of-subset lst2))))))

    (define filter-map
      (lambda (proc lst1 . lst2)
        (if (null? lst2)
          (filter values (map-1/1 proc lst1))
          (filter values (map-n/1 proc (apply list-transpose* lst1 lst2))))))
