(define-library (srfi 26)
  ...
  (begin
    ...
    ; (26-internal-cut slot-names combination . se)
    ...
    (define-syntax 26-internal-cut
      (syntax-rules (<> <...>)

        ;; construct fixed- or variable-arity procedure:
        ;;   (begin proc) throws an error if proc is not an <expression>
        ((26-internal-cut (slot-name ...) (proc arg ...))
          (lambda (slot-name ...) ((begin proc) arg ...)))
        ((26-internal-cut (slot-name ...) (proc arg ...) <...>)
          (lambda (slot-name ... . rest-slot) (apply proc arg ... rest-slot)))

        ;; process one slot-or-expr
        ((26-internal-cut (slot-name ...) (position ...) <> . se)
          (26-internal-cut (slot-name ... x) (position ... x) . se))
        ((26-internal-cut (slot-name ...) (position ...) nse . se)
          (26-internal-cut (slot-name ...) (position ... nse) . se))))

    ; (26-internal-cute slot-names nse-bindings combination . se)
    ;   transformer used internally
    ;     slot-names     : the internal names of the slots
    ...
    (define-syntax 26-internal-cute
      (syntax-rules (<> <...>)
    ...
    (define-syntax cut
      (syntax-rules ()
        ((cut . slots-or-exprs)
          (26-internal-cut () () . slots-or-exprs))))

    (define-syntax cute
      (syntax-rules ()
        ((cute . slots-or-exprs)
          (26-internal-cute () () () . slots-or-exprs))))))
