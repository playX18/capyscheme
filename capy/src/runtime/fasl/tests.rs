    ...
    fn fasl_writer_emits_visit_revisit_uncompressed_group() {
        ...
        scm.enter(|ctx| {
            ...
            assert_eq!(&bytes[12..16], &super::FASL_MACHINE_TYPE.to_le_bytes());
            assert_eq!(&bytes[16..20], &0u32.to_le_bytes()); // lites
            assert_eq!(bytes[20], FASL_TAG_GROUP);
            assert_eq!(bytes[21], FASL_SITUATION_VISIT_REVISIT);
            let group_size = u32::from_le_bytes(bytes[22..26].try_into().unwrap()) as usize;
            assert_eq!(group_size, bytes.len() - 26);
            assert_eq!(bytes[26], FASL_TAG_UNCOMPRESSED);
            let payload_size = u32::from_le_bytes(bytes[27..31].try_into().unwrap()) as usize;
            assert_eq!(payload_size, bytes.len() - 31);
        });
    }
    ...
    fn put_fasl_header(out: &mut Vec<u8>) {
        ...
        out.extend_from_slice(&super::FASL_MACHINE_TYPE.to_le_bytes());
    }
    ...
    fn fasl_writer_emits_loadable_zero_relocation_closure_with_code_block() {
        ...
        scm.enter(|ctx| {
            ...
            // header = 16 bytes (magic 8 + version 4 + machine_type 4)
            assert_eq!(&bytes[0..8], super::FASL_MAGIC);
            assert_eq!(bytes[20], FASL_TAG_GROUP);
            assert_eq!(bytes[21], FASL_SITUATION_VISIT_REVISIT);
            assert_eq!(bytes[26], FASL_TAG_UNCOMPRESSED);
            assert_eq!(bytes[31], FASL_TAG_GRAPH);
            assert_eq!(bytes[40], FASL_TAG_CLOSURE);
            assert_eq!(bytes[41], FASL_TAG_GRAPH_DEF);
