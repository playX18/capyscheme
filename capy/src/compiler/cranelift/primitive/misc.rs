...
pub fn lower_make_tuple<'gc_, 'a, 'f>(
    ...
    let result = ssa.handle_thunk_call_result(ssa.thunks.make_tuple, &[ctx, size, fill]);
...
pub fn lower_make_vector<'gc_, 'a, 'f>(
    ...
    let result = ssa.handle_thunk_call_result(ssa.thunks.make_vector, &[ctx, size, fill]);
...
pub fn lower_string_ref<'gc_, 'a, 'f>(
    ssa: &mut SsaBuilder<'gc_, 'a, 'f>,
    args: &[Atom<'gc_>],
    _source: Value<'gc_>,
) -> PrimValue {
...
pub fn lower_push_cframe<'gc_, 'a, 'f>(
    ...
    let res = ssa
        .builder
        .ins()
        .call(ssa.thunks.push_cframe, &[ctx, key, value, retk]);
...
pub fn lower_current_continuation_marks<'gc_, 'a, 'f>(
    ...
    let result = ssa
        .builder
        .ins()
        .call(ssa.thunks.current_continuation_marks, &[ctx]);
