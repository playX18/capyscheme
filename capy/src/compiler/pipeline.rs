...
pub(crate) fn lower_expanded_to_cps<'gc>(
    ...
    let optimized_il = resolve_free_vars(ctx, il);
    let optimized_il = letrectify(ctx, optimized_il);
    let optimized_il = fix_letrec(ctx, optimized_il);
    let optimized_il = eta_expand(ctx, optimized_il);
    let optimized_il = assignment_elimination::eliminate_assignments(ctx, optimized_il);

    let mut cps = compile_cps::cps_toplevel(ctx, &[optimized_il]);
