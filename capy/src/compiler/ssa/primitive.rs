...
prim!(
    ...
    "lookup-bound-private" => lookup_bound_private(ssa, args, handler) {
        ...
        PrimValue::Value(ssa.handle_thunk_call_result(ssa.thunks.lookup_bound_private, &[ctx, module, name]))
    },
    ...
    "unspecified" => unspec(ssa, _args, _h) {
        let val = ssa.builder.ins().iconst(types::I64, Value::undefined().bits() as i64);
        PrimValue::Value(val)
    },
    ...
    "bytevector?" => is_bytevector(ssa, args, _h) {
        let val = ssa.atom(args[0]);
        let is_bv = ssa.has_any_heap_class_id(val, &[
    ...
    "car" => car(ssa, args, _h) {
        ...
        if !ssa.current_block_has_heap_class_id(pair, builtin_class_ids::PAIR) {
            ...
            ssa.prove_current_block_heap_class_id(pair, builtin_class_ids::PAIR);
    ...
    "#%default-retk" => default_retk(ssa, _args, _h) {
        let ctx = ssa.builder.ins().get_pinned_reg(types::I64);
