...
mod tests {
    ...
    fn lvars_for<'gc>(graph: &Graph<'gc>, vars: &BinderSet) -> Vec<LVarRef<'gc>> {
        vars.iter().map(|var| graph[var].var).collect()
    }
    ...
    fn find_function<'gc>(
        ...
        functions
            ...
            .expect("function binding should be present")
    }

    #[test]
    fn graph_reify_matches_tree_reify_for_nested_closures() {
        Scheme::new_uninit().enter(|ctx| {
            ...
            let tree_info = tree_reify(ctx, func);
            let mut program = cps_func_to_graph(ctx, func).expect("function graph conversion");
            let graph_info = reify_graph(&mut program.graph, program.entry);

            let graph_entry = graph_info.entrypoint;
            let graph_nested = find_function(&program.graph, &graph_info.functions, g);
            let graph_cont = find_function(&program.graph, &graph_info.continuations, k);

            assert!(graph_info.free_vars.function(graph_entry).is_empty());
            assert_eq!(
                lvars_for(&program.graph, graph_info.free_vars.function(graph_nested)),
                vec![k]
            );
            assert_eq!(
                lvars_for(
                    &program.graph,
                    graph_info.free_vars.continuation(graph_cont)
                ),
                vec![ret, x]
            );
            assert!(program.graph[graph_cont].is_reified);
            assert!(cont.reified.get());

            let tree_nested = tree_info
                .functions
                .iter()
                .copied()
                .find(|func| func.binding == g)
                .expect("tree nested function should be present");
            assert_eq!(
                tree_nested.free_vars.get().unwrap().as_slice(),
                &[k],
                "graph and tree reifiers should agree on nested function free vars",
            );
        });
    }

    #[test]
    fn graph_reify_propagates_reified_continuations_transitively() {
        Scheme::new_uninit().enter(|ctx| {
            let f = lvar(ctx, "f");
            let ret = lvar(ctx, "ret");
            ...
            let outer_cont = Gc::new(
                *ctx,
                Cont {
                    ...
                    body: Lock::new(Gc::new(
                        *ctx,
                        Term::Continue(inner, Array::from_slice(*ctx, []), source),
                    )),
                    source,
                    free_vars: Lock::new(None),
                    reified: Cell::new(false),
            ...
            let mut program = cps_func_to_graph(ctx, func).expect("function graph conversion");
            let graph_info = reify_graph(&mut program.graph, program.entry);
            let graph_outer = find_function(&program.graph, &graph_info.continuations, outer);
            let graph_inner = find_function(&program.graph, &graph_info.continuations, inner);

            assert!(program.graph[graph_outer].is_reified);
            assert!(program.graph[graph_inner].is_reified);
        });
    }

    #[test]
    fn graph_reify_marks_primitive_continuation_values() {
        Scheme::new_uninit().enter(|ctx| {
            let f = lvar(ctx, "f");
            let ret = lvar(ctx, "ret");
            let escape = lvar(ctx, "escape");
            let temporary = lvar(ctx, "temporary");
            let source = Value::new(false);
            let prim = Value::new(Symbol::from_str(ctx, "list"));

            let escape_cont = Gc::new(
                *ctx,
                Cont {
                    name: Value::new(false),
                    binding: escape,
            ...
            let body = Gc::new(
                *ctx,
                Term::Letk(
                    Array::from_slice(*ctx, &[escape_cont]),
                    Gc::new(
                        *ctx,
                        Term::Let(
                            temporary,
                            crate::cps::term::Expression::PrimCall(
                                prim,
                                Array::from_slice(*ctx, &[Atom::Local(escape)]),
                                source,
                            ),
                            Gc::new(
                                *ctx,
            ...
            let mut program = cps_func_to_graph(ctx, func).expect("function graph conversion");
            let graph_info = reify_graph(&mut program.graph, program.entry);
            let graph_escape = find_function(&program.graph, &graph_info.continuations, escape);

            assert!(program.graph[graph_escape].is_reified);
        });
    }
}
