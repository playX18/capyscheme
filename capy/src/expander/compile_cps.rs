...
pub fn fixnum_in_range_to_usize<'gc>(
    ...
    ulen: Atom<'gc>,
    h: LVarRef<'gc>,
    have_index_in_range: impl FnOnce(&mut GraphCpsBuilder<'gc>, Subterm, Atom<'gc>) -> TermId,
) -> TermId {
    let bound = cps.fresh_variable("bound");
    let bound_arg = cps.fresh_variable("uidx");
    let bound_body = cps.new_child_link();
    have_index_in_range(cps, bound_body, Atom::Local(bound_arg));
    let bound_cont = cps.make_cont(
        bound,
        &[bound_arg],
        None,
        bound_body,
    ...
    let boundlen = cps.fresh_variable("boundlen");
    let fix = cps.fresh_variable("fix");
    let boundlen_body = cps.new_child_link();
    cps.let_prim(
        boundlen_body,
        "uidx",
        Symbol::from_str(cps.ctx, "s32->u64").into(),
        [Atom::Local(fix)],
        src,
        |cps, owner, uidx| {
            cps.let_prim(
                owner,
                "in_range",
                Symbol::from_str(cps.ctx, "u64<=").into(),
                [Atom::Local(uidx), ulen],
                src,
                |cps, owner, in_range| {
                    cps.emit_if_continue(
                        owner,
    ...
    let boundlen_cont = cps.make_cont(
        boundlen,
        &[fix],
        None,
        boundlen_body,
    ...
    let untag = cps.fresh_variable("untag");
    let untag_body = cps.new_child_link();
    cps.let_prim(
        untag_body,
        "fix",
        Symbol::from_str(cps.ctx, "untag-fixnum").into(),
        [idx],
        src,
        |cps, owner, fix| {
            cps.let_prim(
                owner,
                "below0",
                Symbol::from_str(cps.ctx, "s32<").into(),
                [Atom::Local(fix), Atom::Constant(Value::new(0))],
                src,
                |cps, owner, below0| {
                    cps.emit_if_continue(
                        owner,
    ...
    cps.let_prim(
        body,
        "is_fixnum",
        Symbol::from_str(cps.ctx, "fixnum?").into(),
        [idx],
        src,
        |cps, owner, is_fixnum| {
            cps.emit_if_continue(
                owner,
                Atom::Local(is_fixnum),
                untag,
                &[],
                h,
                &[],
    ...
    cps.emit_letk(
        owner,
        &[bound_cont, boundlen_cont, untag_cont],
        body,
        Value::new(false),
    )
}
